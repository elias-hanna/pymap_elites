#! /usr/bin/env python
# Author Elias Hanna
# Code for NovArchive class and updateNovelty method are taken (or highly inspired) from
# https://github.com/robotsthatdream/diversity_algorithms_dev
...
class NovArchive:
    ...
    def __init__(self, lbd, k=15):
        ...
        if (len(lbd)>0):
            ...
            exit("Error: need a behaviour descriptor to compute novelty")
        self.k=k
    ...
    def get_nov(self,bd, population=[]):
        ...
        if (self.kdtree is None):
            ...
            exit("Error: not working with archive-less NS")
        else:
...
def updateNovelty(population, offspring, archive, params):
   ...
   if (archive) and (archive.size()+len(population)>=k):
       for ind in population:
           if (True in np.isnan(ind.bd)):
               ind.novelty=-1
           else:
